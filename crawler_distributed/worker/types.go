...
func deserializeParser(p SerializedParser) (engine.Parser, error) {
	...
	case config.ParseCityList:
		return engine.NewFuncParser(parserjiaou.ParseCityList, config.ParseCityList), nil
	case config.ParseCity:
		return engine.NewFuncParser(parserjiaou.ParseCity, config.ParseCity), nil
	case config.ParseProfile:
		if userName, ok := p.Args.(string); ok {
			return parserjiaou.NewProfileParser(userName), nil
		} else {
